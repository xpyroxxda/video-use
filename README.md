...
Read install.md first to install this repo, wire up ffmpeg, register the skill with whichever agent you're running under, and configure local faster-whisper transcription. Then read SKILL.md for daily usage, and always read helpers/ because that's where the editing scripts live. After install, don't transcribe anything on your own — just tell me it's ready and wait for me to drop footage into a folder.
```

The agent handles the clone, dependencies, and skill registration. Transcription is local and does not require a paid API key.
...
**Layer 1 — Audio transcript (always loaded).** Local faster-whisper produces word-level timestamps on the user's GPU. All takes pack into a single ~12KB `takes_packed.md` — the LLM's primary reading view. The zero-cost local default does not perform speaker diarization or audio-event tagging.
