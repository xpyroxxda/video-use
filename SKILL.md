...
7. **Pad every cut edge.** Working window: 30–200ms. Local timestamps can drift — padding absorbs the drift. Tighter for fast-paced, looser for cinematic.
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached local word-level JSON
...
First-time install lives in `install.md` (clone, deps, ffmpeg, skill registration, local model). Don't re-run it every session; on cold start just verify:

- Local faster-whisper is installed; for Windows use `scripts/verify-local.ps1` to check CUDA and NVENC.
...
- **`transcribe.py <video>`** — single-file local faster-whisper call. Cached.
- **`transcribe_batch.py <videos_dir>`** — local transcription; one CUDA worker by default. Use for multi-take.
...
- **Running a large local model on CPU unnecessarily.** Use CUDA + float16 on supported NVIDIA hardware; CPU + int8 is the documented fallback.
